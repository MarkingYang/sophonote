...
vi.mock('../../../services/tauri', () => ({
  ...
  disableOpenViking: vi.fn(), restartHermesRuntime: vi.fn(),
}));
import { getOpenVikingStatus, saveOpenVikingConfig, disableOpenViking, restartHermesRuntime } from '../../../services/tauri';
...
describe('OpenViking 设置交互', () => {
  it('requires consent before activation and never reads back a saved key', async () => {
    ...
    expect(saveOpenVikingConfig).toHaveBeenCalledWith(config, '', true);
  ...
  it('disables without deleting data or automatically restarting', async () => {
    ...
    await render();
    await act(async () => button('停用 OpenViking').click());
