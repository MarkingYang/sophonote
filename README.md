...
SophoNote is a local-first AI knowledge workbench for macOS, using open-source [Hermes Agent](https://github.com/NousResearch/hermes-agent), [Pi](https://github.com/earendil-works/pi), and [OpenCode](https://github.com/anomalyco/opencode) as selectable execution engines, with optional locally installed Claude Code. Notes, discovery, permissions, and document review stay in SophoNote. This project is neither a Hermes Desktop fork nor a UI clone.

SophoNote is a **client surface** around pinned agent sidecars:

- Hermes, Pi, Claude Code, and OpenCode share model settings and the chat UI. The first run fixes the engine; selecting another engine creates a new conversation.
- Hermes retains its native Skills/MCP, Browser, and Cron. Pi/Claude/OpenCode use Rust-controlled tools. All four engines can use OpenViking cloud memory after explicit consent.
...
Primary navigation: Discover, Projects, Notes, Scheduled Tasks, Tools. Inbox stays in Settings and is not a top-level entry.

| Surface | Role |
|---|---|
| Discover | Filtered and interpreted AI news, daily reports, and model rankings |
| Projects | Bind a local folder, organize tasks, browse files, and run multi-turn conversations with four engines; existing Studio data is retained and its standalone entry is hidden |
...
- Git contains source, sanitized examples, and sample configuration only. Local databases, conversations, Studio state, notes, logs, private engine sessions, and bundled runtime output are excluded.

Architecture diagram: [current SophoNote architecture](./docs/sophonote-current-architecture.html). Full technical facts: [architecture](./docs/architecture.md) (Chinese).
...
Hermes ships as a pinned sidecar (Hermes 0.20.0 + CPython 3.11). Release builds use the bundled runtime or a verified private update slot, with fallback to the bundled version. Binaries are not in git. How to build the sidecar, attach an external Gateway, pack, and notarize is in [CONTRIBUTING.md](./CONTRIBUTING.md).


New conversations can also select **Pi** below the composer; the engine is fixed after the first Run. Pi 0.85.1 ships as a standalone executable and reuses OpenAI-compatible / Anthropic providers from AI model settings. For source development, run `pnpm pi:bundle` first (Python 3.12+; the initial download requires network access). Pi supports streaming, persistent sessions, cancellation, file approvals, and document diffs; shell commands require approval on macOS. This first integration does not expose Hermes browser, computer control, native Skill/MCP management through Pi.
...
**OpenCode** is also available in the engine menu. Official standalone 1.18.31 ships and updates with the app; source builds require `pnpm opencode:bundle`. It supports OpenAI-compatible / Anthropic models, native session history, streaming, cancellation, and Host-controlled tools and note diffs. Hermes-specific capabilities remain separate.
...
**Cloud memory** is available in Settings → AI configuration → Memory. Connect your own Volcengine OpenViking API Key and confirm data sharing to enable recall and session persistence for all four engines. Browse, read, and edit shared user memory and engine-specific memory. Notes stay local; memory resides in your OpenViking service. Disabling the integration does not delete remote data.
